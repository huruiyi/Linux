  ...
  for (;;) {
    ...
    if (client[0].revents & POLLIN) { /* listenfd有读事件就绪 */
      ...
      for (i = 1; i < OPEN_MAX; i++)
        if (client[i].fd < 0) {
          client[i].fd =
              connfd; /* 找到client[]中空闲的位置,存放accept返回的connfd */
          break;
        }
    ...
    for (i = 1; i <= maxi; i++) { /* 前面的if没满足,说明没有listenfd满足.
                                     检测client[] 看是那个connfd就绪 */
      ...
      if (client[i].revents & POLLIN) {
        ...
        if ((n = Read(sockfd, buf, MAXLINE)) < 0) {
          ...
          if (errno == ECONNRESET) { /* 收到RST标志 */
            ...
            Close(sockfd);
            client[i].fd =
                -1; /* poll中不监控该文件描述符,直接置为-1即可,不用像select中那样移除
                       */
          } else
